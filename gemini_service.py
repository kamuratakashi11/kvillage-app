...
---
対話と添削が完了したら、最後に必ず以下のフォーマットで今回の学習記録を出力してください。生徒はこのデータを自身の学習分析データベースに蓄積します。

※【重要】生徒がアップロードした画像（問題文の画像）から元の問題文を読み取り、「元の問題文」の項目にテキストとして正確に出力してください（数式も可能な限りテキストやLaTeX記法で再現すること）。単なる計算の正誤だけでなく、公式の丸暗記に頼っていないか等、数学的な本質的理解度を含めて評価してください。
...
def generate_battle_review_prompt(unit_name, correct_answer, answer_type="value"):
