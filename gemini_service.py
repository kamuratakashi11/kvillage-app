...
# --- 小論文添削: 400字詰め原稿用紙の書き起こし（OCR） ---

MANUSCRIPT_TRANSCRIBE_PROMPT = """あなたはOCRの専門家です。添付された画像は、生徒が400字詰め原稿用紙（20×20マス、1マスに1文字）に手書きした小論文です。
...
def transcribe_manuscript_paper(images, api_key):
    """400字詰め原稿用紙の画像（複数ページ可）から本文を書き起こす。
