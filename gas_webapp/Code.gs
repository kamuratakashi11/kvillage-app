...
var CORRECTION_PROMPT = [
  ...
  '4. **対話の継続**: 生徒から追加の質問が来ることを想定し、一方的に終わらせず、対話を続ける姿勢で答えてください。',
  '5. **簡潔さ**: 同じ指摘・結論を言い回しを変えて何度も繰り返さないでください。一つの論点は一度だけ、簡潔に述べてください。',
  '',
  '# トーン＆スタイル（Tone & Style）',
...
function callGemini_(base64Image, mimeType) {
  ...
  var payload = {
    ...
    // maxOutputTokensを指定しないと、モデルが同じ指摘を言い回しを変えて延々と繰り返す
    // 「反復退化」を起こし、分析ノートDocsが際限なく膨張することがあるため上限を設ける
    generationConfig: {
