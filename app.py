...
def main():
    ...
    if rpg_field_from_url and rpg_dungeon_from_url in rpg_data.CATEGORY_MAP:
        ...
        st.title("🤖 AI自動添削")
        st.write("解いた問題の写真をアップロードすると、AIが自動で添削し、結果をあなた専用の分析ノート（Google Docs）に自動で書き足してくれます。")

        gas_webapp_url = st.secrets.get("GAS_WEBAPP_URL", "")
        gas_hmac_secret = st.secrets.get("GAS_HMAC_SECRET", "")

        if not gas_webapp_url or not gas_hmac_secret:
            st.error(
                "⚠️ AI自動添削機能はまだ設定が完了していません。"
                "先生（管理者）に、`GAS_WEBAPP_URL`と`GAS_HMAC_SECRET`の設定（Secrets）を"
                "お願いしてください。"
            )
        else:
...
1. 「🤖 AI自動添削」ページに表示されている、あなた専用の「分析ノート（Google Docs）」のリンクを開きます。
2. Docsの画面上部の「ファイル」メニューから「ショートカットをドライブに追加」（またはファイル名の右にある＋アイコン）を選び、**自分のGoogle Driveの「マイドライブ」に追加**します。（この分析ノートは先生のアカウントで自動作成されたものなので、一度この操作をしないと、あなたのドライブの中からは見つけられません。最初の1回だけでOKです）
3. **NotebookLM**（ https://notebooklm.google.com/ ）を開き、必ず**個人のGoogleアカウント**でログインします。
4. 「新しいノートブック」を作成し、名前に「数学 弱点分析ノート」などと入力します。
5. 「ソースを追加」の画面が出るので、**「Google ドライブ」**を選びます。
6. 手順2でマイドライブに追加した「分析ノート（Google Docs）」を選択して追加します。
7. ノートブックの設定（右上の設定ボタン等）から、「カスタムのノートブックの概要を設定」を開き、以下の指示をコピーして貼り付け、保存します。

> `あなたは生徒の学習をサポートする優秀な数学コーチです。ソースとして提供されている「学習記録データ（元の問題文、理解度、弱点分析など）」を読み込み、以下の指示に従って「理解度チェックテスト」を出力してください。`
>
> `【出力形式】`
> `1. **現在の全体的な分析**: ソースから見える、生徒の現在の「弱点」や「思考の癖」を簡潔に分析してください。`
> `2. **オリジナル復習テスト**: 生徒の過去の弱点を克服するための問題を1問作成してください。出力は必ず以下のテンプレートを守ってください。`
>
> `---`
> `【問題】（ここに、生徒が挑戦すべき良問を、数式や条件を明確にして記述してください）`
>
> `（ここに、生徒が取り組み始めるための「図を描くヒント」や「最初のステップへの誘導」を、優しく励ますトーンで記述してください）`
> `---`
>
> `【重要】`
> `* 図形問題のように視覚情報が必要な場合は、その図形を言葉で論理的に説明し、作図を誘導してください。`
> `* あなたは提供されたソースの中だけで回答するAIです。インターネット検索などの外部リソースを利用した提案は行わず、手元にあるデータのみを使ってテスト作成に集中してください。`

---

**【週末のルーティン】テストの自動生成**
毎日の学習では、「🤖 AI自動添削」ページで写真をアップロードするだけでOKです（分析ノートへの追記は自動で行われます）。NotebookLMを開くのは週末やテスト前です！
